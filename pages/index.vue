<template>
  <div class="container">
    <div class="page">
      ...
      <div class="footer">
        Карта бизнеса, терпящего бедствие © 2020 <br/>
        <span>Сделать карту тонущего бизнеса предложил предприниматель <span style="white-space: nowrap">Михаил Зуйков</span></span>
        <br/><br/>
        <span>Дизайн: <a style="color: black; text-decoration: none;" target="_blank" href="https://tlgg.ru/nickk2007">Никита Куренев</a></span>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        center: [55.75541342220562,37.62138633319637],
        form: {
          email: '',
          address: '',
          coords: [],
    ...
    methods: {
      submitForm() {
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            ...
            this.$fireStore.collection("business").doc().set({...data, email: null}).then((doc) => {
      ...
      async fetchData() {
        this.$fireStore.collection("business").get().then((querySnapshot) => {
          this.placemarkData = querySnapshot.docs.map((doc) => doc.data())
        })
      }
    }
  }
